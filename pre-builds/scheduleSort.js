...
export function sortByDay(schedule) {
    ...
    if (dayListShortcut == null) throw "Schedule lacks conventional structure, should contain schedule[program][days]"
    dayListShortcut.sort((dict1, dict2) => {
        const day1 = dict1["day"];
        const day2 = dict2["day"];
        if (day1 == null || day2 == null) throw "Dicts in list of days doesn't contain key = 'day'";
        if (!dayOfWeekOrder.includes(day1)) throw `Day: ${day1} is not a valid day of week, must be capitalized english`;
        if (!dayOfWeekOrder.includes(day2)) throw `Day: ${day2} is not a valid day of week, must be capitalized english`;
...
export function sortByTimeStart(schedule) {
    ...
    for (var i = 0; i < dayListShortcut.length; i++) { 
        ...
        if (slotListShortcut == null) throw "Schedule lacks conventional structure, should contain schedule[program][days][slots]";
        ...
        slotListShortcut.sort((dict1, dict2) => {
            ...
            if (timeStart1 == null || timeStart2 == null) throw "Dicts in list of slots doesn't contain key = 'timeStart'";
